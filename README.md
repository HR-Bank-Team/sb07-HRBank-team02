...
### 📝 수정이력관리
- **동적 검색 조건 처리 방식 개선**
    - **문제:** 사번, 기간, 유형, IP 등 선택적으로 적용되는 검색 조건을 JPQL만으로 처리하기 어려움
    - **발생 원인:** JPQL을 문자열 기반으로 구성하면서 조건이 추가될수록 분기 로직이 복잡해지고, 가독성과 유지보수성이 저하됨
    - **해결:** 수동으로 동적 쿼리를 구성하여 조건별 쿼리 생성 로직을 분리하고, 복합 조건을 유연하게 조합할 수 있도록 구조 개선
    - **배운 점:** Querydsl의 필요성과 장점을 이해함
---
